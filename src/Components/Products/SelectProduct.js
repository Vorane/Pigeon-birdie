...
class SelectProduct extends Component {
  ...
  componentDidUpdate(prevProps, prevState, snapShot) {
    if (
      prevProps.selectOrderItemProcess.status !== processTypes.SUCCESS &&
      this.props.selectOrderItemProcess.status === processTypes.SUCCESS
    ) {
      //close
      setTimeout(() => {
  ...
  render() {
    ...
    let { selectOrderItemProcess } = this.props;
    let showIdle = selectOrderItemProcess.status === processTypes.IDLE;
    let showLoading = selectOrderItemProcess.status === processTypes.PROCESSING;
    let showSuccess = selectOrderItemProcess.status === processTypes.SUCCESS;
    let showError = selectOrderItemProcess.status === processTypes.ERROR;
    return (
      <Wrapper>
        <SearchOutletContainer>
          ...
          <SearchOutletProduct
            {...this.props}
            onProductPress={this._onProductPress}
          />
        </SearchOutletContainer>
        {productSelected && (
          <Fragment>
            {showIdle ? (
              <ButtonFooter
                buttonText={"Swap"}
                pressHandler={this._onPressSwap}
              >
                <ProductContainer>
